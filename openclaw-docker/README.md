...
#    the only secret is OPENCLAW_GATEWAY_TOKEN, which is generated during
#    first-run onboarding. Leave it blank to let onboarding create one, or
#    paste in an existing token:
#      OPENCLAW_GATEWAY_TOKEN=<token-from-onboarding>
$EDITOR .env
...
- `.env` is gitignored. Never commit real gateway tokens.
